# SpringIOC Resource和Autowired的区别
...
public class TestServiceImpl {    // 下面两种@Autowired只要使用一种即可    @Autowired    private UserDao userDao; // 用于字段上    
...
public class TestServiceImpl {    // 下面两种@Resource只要使用一种即可
    ...
    @Resource(name="userDao")    public void setUserDao(UserDao userDao) { // 用于属性的setter方法上
        this.userDao = userDao;
    }
}
